...
var Tokenizer = class {
	...
	*parseTokens(quote) {
		...
		while (this.pos < endIndex) {
			...
			} else if (cp === CP_CR$1 || cp === CP_LF$1) if (inTemplate) {
				if (cp === CP_CR$1 && this.source.codePointAt(this.pos) === CP_LF$1) this.pos++;
				yield {
					type: "CharacterToken",
					value: "\n",
					range: [start, this.pos]
	...
	readEscape(inTemplate) {
		...
		switch (cp) {
			...
			case CP_CR$1: if (this.source.codePointAt(this.pos) === CP_LF$1) this.pos++;
			case CP_LF$1: return {
				value: "",
				kind: "eol"
			};
...
function* iterateReferencesForVariable(identifier, context, alreadyChecked) {
	...
	for (const reference of readReferences) yield* iterateReferencesForExpression(reference.identifier, context, alreadyChecked);
}
...
function isWellKnownArrayMethodName(name$2) {
	return Boolean(WELL_KNOWN_ARRAY_METHODS[name$2]);
}

//#endregion
//#region lib/utils/get-usage-of-pattern.ts
let UsageOfPattern = /* @__PURE__ */ function(UsageOfPattern$1) {
	/** The pattern was only used via `.source`. */
	UsageOfPattern$1[UsageOfPattern$1["partial"] = 0] = "partial";
	/** The pattern was (probably) used the whole pattern as a regular expression. */
	UsageOfPattern$1[UsageOfPattern$1["whole"] = 1] = "whole";
	/** The pattern was used partial and whole. */
	UsageOfPattern$1[UsageOfPattern$1["mixed"] = 2] = "mixed";
	/** The pattern cannot determine how was used. */
	UsageOfPattern$1[UsageOfPattern$1["unknown"] = 3] = "unknown";
	return UsageOfPattern$1;
}({});
/**
...
let EscapeSequenceKind = /* @__PURE__ */ function(EscapeSequenceKind$1) {
	EscapeSequenceKind$1["octal"] = "octal";
	EscapeSequenceKind$1["control"] = "control";
	EscapeSequenceKind$1["hexadecimal"] = "hexadecimal";
	EscapeSequenceKind$1["unicode"] = "unicode";
	EscapeSequenceKind$1["unicodeCodePoint"] = "unicode code point";
	return EscapeSequenceKind$1;
}({});
/**
...
const FLAG_GLOBAL = "g";
const FLAG_DOT_ALL = "s";
const FLAG_HAS_INDICES = "d";
const FLAG_IGNORECASE = "i";
const FLAG_MULTILINE = "m";
const FLAG_STICKY = "y";
const FLAG_UNICODE = "u";
const FLAG_UNICODE_SETS = "v";
...
*/
function canSimplifyQuantifier(quantifier, flags, parser$1) {
...
function canAbsorb(initialPreceding, options) {
	const { direction, flags, parser: parser$1, quantifier } = options;
	...
	const formal = everyMaybe(fast, (q) => canAbsorbElementFormal(q, quantifier.element, parser$1));
...
function toNfa(element, parser$1) {
	const { expression, maxCharacter } = parser$1.parseElement(element, {
		maxNodes: 1e3,
		assertions: "throw",
...
*/
function canAbsorbElementFormal(quantifier, element, parser$1) {
	if (containsAssertions$1(quantifier) || containsAssertions$1(element)) return;
	try {
		const qNfa = toNfa(quantifier, parser$1);
		const qDfa = DFA.fromFA(qNfa, new DFA.LimitedNodeFactory(1e3));
		const eNfa = toNfa(element, parser$1);
		eNfa.quantify(0, 1);
		qNfa.append(eNfa);
...
function getPrecedingQuantifiers(element, direction, flags) {
	...
	const inc$1 = direction === "ltr" ? -1 : 1;
	const { elements } = parent;
	const elementIndex = elements.indexOf(element);
	for (let precedingIndex = elementIndex + inc$1; precedingIndex >= 0 && precedingIndex < elements.length; precedingIndex += inc$1) {
...
function isArrayLikeObject(tsType) {
	const ts$1 = getTypeScript();
	return isObject(tsType) && (tsType.objectFlags & (ts$1.ObjectFlags.ArrayLiteral | ts$1.ObjectFlags.EvolvingArray | ts$1.ObjectFlags.Tuple)) !== 0;
}
/**
...
function isClassOrInterface(tsType) {
	const ts$1 = getTypeScript();
	return isObject(tsType) && (tsType.objectFlags & ts$1.ObjectFlags.ClassOrInterface) !== 0;
}
/**
...
function isObject(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.Object) !== 0;
}
/**
...
function isReferenceObject(tsType) {
	const ts$1 = getTypeScript();
	return isObject(tsType) && (tsType.objectFlags & ts$1.ObjectFlags.Reference) !== 0;
}
/**
...
function isUnionOrIntersection(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.UnionOrIntersection) !== 0;
}
/**
...
function isTypeParameter(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.TypeParameter) !== 0;
}
/**
...
function isAny(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.Any) !== 0;
}
/**
...
function isUnknown(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.Unknown) !== 0;
}
/**
...
function isStringLine(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.StringLike) !== 0;
}
/**
...
function isNumberLike(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.NumberLike) !== 0;
}
/**
...
function isBooleanLike(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.BooleanLike) !== 0;
}
/**
...
function isBigIntLike(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.BigIntLike) !== 0;
}
/**
...
function isNull(tsType) {
	const ts$1 = getTypeScript();
	return (tsType.flags & ts$1.TypeFlags.Null) !== 0;
}
...
var JSDocParams = class {
	...
	add(paths, param) {
		const name$2 = paths.shift();
		if (paths.length > 0) {
			for (const rootParam of this.params) if (rootParam.name === name$2) {
				rootParam.add(paths, param);
				return;
			}
		}
		this.params.push(new JSDocParam(name$2 || null, param));
	}
	get(paths) {
		const { name: name$2, index } = paths.shift();
		if (name$2) {
			for (const param of this.params) if (param.name === name$2) return paths.length ? param.get(paths) : param.param;
		}
		if (index != null) {
...
var JSDoc = class {
	...
	getTag(name$2) {
		for (const tag of this.genTags(name$2)) return tag;
		return null;
	}
	parseParams() {
...
var TypeObject = class {
	...
	*allProperties() {
	...
	propertyType(name$2) {
		for (const [key, getValue] of this.allProperties()) if (key === name$2) return getValue();
		return getObjectPrototypes()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
var TypeNumber = class {
	...
	propertyType(name$2) {
		return getPrototypes$9()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
var TypeString = class {
	...
	propertyType(name$2) {
		if (name$2 === "0") return this;
		return getPrototypes$8()[name$2] || null;
	}
	iterateType() {
		return this;
	}
...
var TypeBigInt = class {
	...
	propertyType(name$2) {
		return getPrototypes$7()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
var TypeBoolean = class {
	...
	propertyType(name$2) {
		return getPrototypes$6()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
var TypeRegExp = class {
	...
	propertyType(name$2) {
		return getPrototypes$5()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
var TypeFunction = class {
	...
	returnType(thisType, argTypes, meta$1) {
		return this.fn(thisType, argTypes, meta$1);
	}
	paramType() {
		return null;
	}
	propertyType(name$2) {
		return getPrototypes$4()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
var TypeGlobalFunction = class extends TypeFunction {
	...
	propertyType(name$2) {
		return this.props[name$2] || super.propertyType(name$2);
	}
};
...
var TypeIterable = class {
	...
	propertyType(name$2) {
		return getPrototypes$3()[name$2] || null;
	}
	iterateType() {
	...
	propertyType(name$2) {
		...
		return TypeUnionOrIntersection.buildType(function* () {
			for (const type of baseCollection.all()) {
				const propType = isTypeClass(type) ? type.propertyType(name$2) : null;
...
var TypeArray = class {
	...
	propertyType(name$2) {
		if (name$2 === "0") return this.paramType(0);
		return getPrototypes$2()[name$2] || null;
	}
	iterateType() {
		return this.paramType(0);
	}
...
const getPrototypes$2 = lazy(() => {
	...
	const RETURN_SELF$1 = new TypeFunction(
		/**
		* Function Type that Return self array
		*/
		function returnSelf(selfType) {
	...
	return createObject({
		...
		join: RETURN_STRING,
		reverse: RETURN_SELF$1,
		shift: RETURN_ARRAY_ELEMENT,
		slice: RETURN_SELF$1,
		sort: RETURN_SELF$1,
		splice: RETURN_SELF$1,
		...
		toReversed: RETURN_SELF$1,
		toSorted: RETURN_SELF$1,
		toSpliced: RETURN_SELF$1,
		with: RETURN_SELF$1,
		length: NUMBER,
		0: null,
		[Symbol.iterator]: null,
...
var TypeMap = class {
	...
	propertyType(name$2) {
		return getPrototypes$1()[name$2] || null;
	}
	iterateType() {
		const map = this;
		return new TypeArray(function* () {
...
*/
function mapConstructor(_thisType, argTypes, meta$1) {
	if (!meta$1?.isConstructor) return null;
...
const getPrototypes = lazy(() => {
	...
	return createObject({
		...
		[Symbol.iterator]: null,
		[Symbol.toStringTag]: STRING,
		difference: RETURN_SELF$1,
		intersection: RETURN_SELF$1,
		isDisjointFrom: RETURN_BOOLEAN,
		isSubsetOf: RETURN_BOOLEAN,
		isSupersetOf: RETURN_BOOLEAN,
		symmetricDifference: RETURN_SELF$1,
		union: RETURN_SELF$1
	});
});
var TypeSet = class {
	...
	propertyType(name$2) {
		return getPrototypes()[name$2] || null;
	}
	iterateType() {
		return this.paramType(0);
	}
...
*/
function setConstructor(_thisType, argTypes, meta$1) {
	if (!meta$1?.isConstructor) return null;
	const arg = argTypes[0]?.();
...
var TypeGlobal = class {
	...
	propertyType(name$2) {
		return getProperties()[name$2] || null;
	}
	iterateType() {
		return null;
	}
...
function createTypeTracker(context) {
	...
	const cache$1 = cacheTypeTracker.get(programNode);
		...
		if (node.type === "ArrayExpression") return new TypeArray(function* () {
			...
			for (let index = node.properties.length - 1; index >= 0; index--) {
				...
				if (property.type !== "SpreadElement") {
					...
					const name$2 = getPropertyName(context, property);
					if (name$2 != null) {
						const value = property.value;
						yield [name$2, () => getType(value)];
					}
				} else {
	...
	*/
	function getTypeFromTsType(tsType) {
		...
		if (isClassOrInterface(tsType)) {
			const name$2 = tsType.symbol.escapedName;
			const typeName = /^Readonly(?<typeName>.*)/u.exec(name$2)?.groups.typeName ?? name$2;
...
function jsDocTypeNodeToTypeInfo(node) {
	...
	if (node.type === "JsdocTypeTuple") {
		if (node.elements[0].type === "JsdocTypeKeyValue") {
			const elements$1 = node.elements;
			return new TypeArray(function* () {
				for (const element of elements$1) if (element.right) yield jsDocTypeNodeToTypeInfo(element.right);
			});
		}
...
function typeNameToTypeInfo(name$2) {
	if (name$2 === "String" || name$2 === "string") return STRING;
	if (name$2 === "Number" || name$2 === "number") return NUMBER;
	if (name$2 === "Boolean" || name$2 === "boolean") return BOOLEAN;
	if (name$2 === "BigInt" || name$2 === "bigint") return BIGINT;
	if (name$2 === "RegExp") return REGEXP;
	if (name$2 === "Array" || name$2 === "array") return UNKNOWN_ARRAY;
	if (name$2 === "Function" || name$2 === "function") return UNKNOWN_FUNCTION;
	if (name$2 === "Object" || name$2 === "object") return UNKNOWN_OBJECT;
	if (name$2 === "Record") return UNKNOWN_OBJECT;
	if (name$2 === "Map") return UNKNOWN_MAP;
	if (name$2 === "Set") return UNKNOWN_SET;
	if (name$2 === "Generator" || name$2 === "Iterable" || name$2 === "IterableIterator") return UNKNOWN_ITERABLE;
	return null;
}
/**
* Get function param path for param node
*/
function getParamPath(name$2, node, context) {
	const parent = getParent(node);
	if (!parent) return [{
		name: name$2,
		index: null
	...
	if (parent.type === "AssignmentPattern") return getParamPath(name$2, parent, context);
	...
	if (parent.type === "RestElement") return getParamPath(name$2, parent, context);
	return [{
		name: name$2,
		index: null
	}];
...
const CP_HASH = "#".codePointAt(0);
const CP_DOLLAR = "$".codePointAt(0);
const CP_PERCENT = "%".codePointAt(0);
const CP_AMP = "&".codePointAt(0);
...
const CP_COMMA = ",".codePointAt(0);
const CP_MINUS = "-".codePointAt(0);
const CP_DOT = ".".codePointAt(0);
const CP_SLASH = "/".codePointAt(0);
const CP_COLON = ":".codePointAt(0);
const CP_SEMI = ";".codePointAt(0);
const CP_LT = "<".codePointAt(0);
const CP_EQ = "=".codePointAt(0);
const CP_GT = ">".codePointAt(0);
...
const CP_NBSP = "\xA0".codePointAt(0);
const CP_OGHAM_SPACE_MARK = " ".codePointAt(0);
...
const CP_LINE_SEPARATOR = "\u2028".codePointAt(0);
const CP_PARAGRAPH_SEPARATOR = "\u2029".codePointAt(0);
const CP_NNBSP = " ".codePointAt(0);
const CP_MMSP = " ".codePointAt(0);
const CP_BRAILLE_PATTERN_BLANK = "⠀".codePointAt(0);
const CP_IDEOGRAPHIC_SPACE = "　".codePointAt(0);
const CP_BOM = "﻿".codePointAt(0);
...
function defineRegexpVisitor(context, rule) {
	const programNode = context.sourceCode.ast;
	let visitor;
	let rules$4 = regexpRules.get(programNode);
	if (!rules$4) {
		rules$4 = [];
		regexpRules.set(programNode, rules$4);
		visitor = buildRegexpVisitor(context, rules$4, () => {
			regexpRules.delete(programNode);
		});
	} else visitor = {};
	let createLiteralVisitor = void 0;
	let createSourceVisitor = void 0;
...
function buildRegexpVisitor(context, rules$4, programExit) {
	...
	*/
	function verify(patternNode, flagsNode, regexpNode, patternSource, flagsString, ownsFlags, createVisitor) {
		...
		let parsedPattern;
		try {
			parsedPattern = parser$1.parsePattern(patternSource.value, 0, patternSource.value.length, flags);
		} catch (error) {
			if (error instanceof SyntaxError) visitInvalidForRules(rules$4, {
	...
	return {
		...
		Literal(node) {
			...
			verify(node, node, node, PatternSource.fromRegExpLiteral(context, node), flagsString, true, (base) => {
				return createLiteralVisitorFromRules(rules$4, {
					node,
					flagsString,
					ownsFlags: true,
					regexpNode: node,
		...
		Program(program) {
			...
			for (const { call, patternNode, patternSource, flagsNode, flagsString, ownsFlags } of regexpDataList) verify(patternNode, flagsNode, call, patternSource, flagsString, ownsFlags, (base) => {
				return createSourceVisitorFromRules(rules$4, {
					node: patternNode,
					flagsString,
					ownsFlags,
					regexpNode: call,
...
function createLiteralVisitorFromRules(rules$4, context) {
	const handlers = [];
	for (const rule of rules$4) if (rule.createLiteralVisitor) handlers.push(rule.createLiteralVisitor(context));
	return composeRegExpVisitors(handlers);
}
/** Create a visitor handler from the given rules */
function createSourceVisitorFromRules(rules$4, context) {
	const handlers = [];
	for (const rule of rules$4) if (rule.createSourceVisitor) handlers.push(rule.createSourceVisitor(context));
	return composeRegExpVisitors(handlers);
}
/** Calls a visit function for all the given rules */
function visitInvalidForRules(rules$4, context) {
	for (const rule of rules$4) rule.visitInvalid?.(context);
}
/** Calls a visit function for all the given rules */
function visitUnknownForRules(rules$4, context) {
	for (const rule of rules$4) rule.visitUnknown?.(context);
}
...
var control_character_escape_default = createRule("control-character-escape", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 } = regexpContext;
			return { onCharacterEnter(cNode) {
				...
				context.report({
					...
					fix: fixReplaceNode$1(cNode, expectedRaw)
				});
			} };
		}
		return defineRegexpVisitor(context, { createVisitor });
...
var hexadecimal_escape_default = createRule("hexadecimal-escape", {
	...
	create(context) {
		...
		function verifyForAlways({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }, kind, cNode) {
			...
			context.report({
				...
				fix: fixReplaceNode$1(cNode, hexEscape)
			});
		}
		function verifyForNever({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }, kind, cNode) {
			...
			context.report({
				...
				fix: fixReplaceNode$1(cNode, () => `\\u00${cNode.raw.slice(2)}`)
			});
		}
...
var letter_case_default = createRule("letter-case", {
	...
	create(context) {
		...
		function report({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }, reportNode, letterCase, convertText) {
			context.report({
				node,
				...
				fix: fixReplaceNode$1(reportNode, () => convertText(CONVERTER[letterCase]))
			});
		}
...
var negation_default = createRule("negation", {
	...
	create(context) {
		function createVisitor({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, flags }) {
			return { onCharacterClassEnter(ccNode) {
				...
				context.report({
					...
					messageId: "unexpected",
					data: { negatedCharSet },
					fix: fixReplaceNode$1(ccNode, negatedCharSet)
				});
			} };
		}
		return defineRegexpVisitor(context, { createVisitor });
...
function* getNextElements(start, dir, flags) {
	...
	for (;;) {
		...
		const inc$1 = dir === "ltr" ? 1 : -1;
		for (let i = index + inc$1; i >= 0 && i < elements.length; i += inc$1) {
			const e = elements[i];
			yield e;
...
function tryFindContradictionInAlternative(alternative, dir, condition, flags) {
	...
	const inc$1 = dir === "ltr" ? 1 : -1;
	for (let i = first; i >= 0 && i < elements.length; i += inc$1) {
		const e = elements[i];
...
var no_contradiction_with_assertion_default = createRule("no-contradiction-with-assertion", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, flags, getRegexpLocation, fixReplaceQuant: fixReplaceQuant$1, fixReplaceNode: fixReplaceNode$1 } = regexpContext;
...
var no_control_character_default = createRule("no-control-character", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, patternSource, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 } = regexpContext;
			...
			return { onCharacterEnter(cNode) {
				if (cNode.value <= 31 && isBadEscape(cNode)) {
					...
					if (allowedEscape !== void 0) suggest.push({
						...
						fix: fixReplaceNode$1(cNode, allowedEscape)
					});
					context.report({
						node,
						loc: getRegexpLocation(cNode),
...
*/
function toNFA(parser$1, element) {
	try {
		const { expression, maxCharacter } = parser$1.parseElement(element, {
		...
		return {
			nfa: NFA.empty({ maxCharacter: parser$1.maxCharacter }),
			partial: true
		};
	}
}
...
*/
function* iteratePartialAlternatives(alternative, parser$1) {
	if (isNonRegular(alternative)) return;
	const maxCharacter = parser$1.maxCharacter;
	const partialParser = new PartialParser(parser$1, {
		assertions: "throw",
		backreferences: "throw"
	});
...
var SubsetRelation = /* @__PURE__ */ function(SubsetRelation$1) {
	SubsetRelation$1[SubsetRelation$1["none"] = 0] = "none";
	SubsetRelation$1[SubsetRelation$1["leftEqualRight"] = 1] = "leftEqualRight";
	SubsetRelation$1[SubsetRelation$1["leftSubsetOfRight"] = 2] = "leftSubsetOfRight";
	SubsetRelation$1[SubsetRelation$1["leftSupersetOfRight"] = 3] = "leftSupersetOfRight";
	SubsetRelation$1[SubsetRelation$1["unknown"] = 4] = "unknown";
	return SubsetRelation$1;
}(SubsetRelation || {});
...
*/
function* findPrefixDuplicationNfa(alternatives, parser$1) {
	if (alternatives.length === 0) return;
	const all$1 = NFA.all({ maxCharacter: alternatives[0][0].maxCharacter });
	for (let i = 0; i < alternatives.length; i++) {
		...
		if (!partial) {
			...
			if (overlapping.length >= 1) {
				...
				if (isSubsetOf(othersNfa, nfa)) yield {
					...
					const nested = tryFindNestedSubsetResult(overlapping.map((o) => [o[0], o[2]]), othersNfa, alternative, parser$1);
					if (nested) yield {
						...nested,
...
* Tries to find duplication in the given alternatives.
*/
function* findDuplicationNfa(alternatives, flags, { hasNothingAfter, parser: parser$1, ignoreOverlap }) {
	const previous = [];
	for (let i = 0; i < alternatives.length; i++) {
		const alternative = alternatives[i];
		const { nfa, partial } = toNFA(parser$1, alternative);
	...
	if (hasNothingAfter) yield* findPrefixDuplicationNfa(previous, parser$1);
}
/**
...
*/
function tryFindNestedSubsetResult(others, othersNfa, alternative, parser$1) {
	const disjointElements = /* @__PURE__ */ new Set();
	for (const { nested, nfa: nestedNfa } of iteratePartialAlternatives(alternative, parser$1)) {
...
var ReportOption = /* @__PURE__ */ function(ReportOption$1) {
	ReportOption$1["all"] = "all";
	ReportOption$1["trivial"] = "trivial";
	ReportOption$1["interesting"] = "interesting";
	return ReportOption$1;
}(ReportOption || {});
var ReportExponentialBacktracking = /* @__PURE__ */ function(ReportExponentialBacktracking$1) {
	ReportExponentialBacktracking$1["none"] = "none";
	ReportExponentialBacktracking$1["certain"] = "certain";
	ReportExponentialBacktracking$1["potential"] = "potential";
	return ReportExponentialBacktracking$1;
}(ReportExponentialBacktracking || {});
var ReportUnreachable = /* @__PURE__ */ function(ReportUnreachable$1) {
	ReportUnreachable$1["certain"] = "certain";
	ReportUnreachable$1["potential"] = "potential";
	return ReportUnreachable$1;
}(ReportUnreachable || {});
var MaybeBool = /* @__PURE__ */ function(MaybeBool$1) {
	MaybeBool$1[MaybeBool$1["false"] = 0] = "false";
	MaybeBool$1[MaybeBool$1["true"] = 1] = "true";
	MaybeBool$1[MaybeBool$1["maybe"] = 2] = "maybe";
	return MaybeBool$1;
}(MaybeBool || {});
var no_dupe_disjunctions_default = createRule("no-dupe-disjunctions", {
	...
	create(context) {
		...
		function createVisitor(regexpContext) {
			const { flags, node, getRegexpLocation, getUsageOfPattern: getUsageOfPattern$1 } = regexpContext;
			const parser$1 = getParser(regexpContext);
			/** Returns the filter information for the given node */
			function getFilterInfo(parentNode) {
				const usage = getUsageOfPattern$1();
				let stared;
...
var no_empty_alternative_default = createRule("no-empty-alternative", {
	...
	create(context) {
		function createVisitor({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }) {
				...
				if (regexpNode.alternatives.length >= 2) for (let i = 0; i < regexpNode.alternatives.length; i++) {
					...
					if (alt.elements.length === 0) {
						...
						context.report({
							...
							suggest: fixed ? [{
								messageId: "suggest",
								fix: fixReplaceNode$1(regexpNode, fixed)
							}] : void 0
						});
						return;
					}
...
var no_escape_backspace_default = createRule("no-escape-backspace", {
	...
	create(context) {
		function createVisitor({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }) {
			return { onCharacterEnter(cNode) {
				if (cNode.value === CP_BACKSPACE && cNode.raw === "\\b") context.report({
					node,
					loc: getRegexpLocation(cNode),
					messageId: "unexpected",
					suggest: [{
						messageId: "suggest",
						fix: fixReplaceNode$1(cNode, "\\u0008")
					}]
				});
			} };
		}
...
var no_extra_lookaround_assertions_default = createRule("no-extra-lookaround-assertions", {
	...
	create(context) {
		...
		function reportLookaroundAssertion({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }, assertion) {
			...
			context.report({
				...
				fix: fixReplaceNode$1(assertion, replaceText)
			});
		}
		return defineRegexpVisitor(context, { createVisitor });
	}
...
var no_invalid_regexp_default = createRule("no-invalid-regexp", {
	...
	create(context) {
		...
		function visitUnknown(regexpContext) {
			const { node, flags, flagsString, getFlagsLocation: getFlagsLocation$1 } = regexpContext;
			const flagSet = /* @__PURE__ */ new Set();
			for (const flag of flagsString ?? "") {
				if (flagSet.has(flag)) {
					context.report({
						node,
						loc: getFlagsLocation$1(),
...
var no_invisible_character_default = createRule("no-invisible-character", {
	...
	create(context) {
		const sourceCode = context.sourceCode;
		function createLiteralVisitor({ node, flags, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }) {
			return { onCharacterEnter(cNode) {
				if (cNode.raw === " ") return;
				if (cNode.raw.length === 1 && isInvisible(cNode.value)) {
					...
					context.report({
						...
						messageId: "unexpected",
						data: { instead },
						fix: fixReplaceNode$1(cNode, instead)
					});
				}
			} };
		}
...
var no_lazy_ends_default = createRule("no-lazy-ends", {
	...
	create(context) {
		...
		function createVisitor({ node, getRegexpLocation, getUsageOfPattern: getUsageOfPattern$1, fixReplaceNode: fixReplaceNode$1 }) {
			if (ignorePartial) {
				if (getUsageOfPattern$1() !== UsageOfPattern.whole) return {};
			}
			return { onPatternEnter(pNode) {
				for (const lazy$1 of extractLazyEndQuantifiers(pNode.alternatives)) {
					const makeGreedy$1 = {
						messageId: "suggestMakeGreedy",
						fix: fixReplaceNode$1(lazy$1, lazy$1.raw.slice(0, -1))
					};
					if (lazy$1.min === 0) {
						const replacement = pNode.alternatives.length === 1 && pNode.alternatives[0].elements.length === 1 && pNode.alternatives[0].elements[0] === lazy$1 ? "(?:)" : "";
						context.report({
							node,
							loc: getRegexpLocation(lazy$1),
							messageId: "uselessElement",
							suggest: [{
								messageId: "suggestRemoveElement",
								fix: fixReplaceNode$1(lazy$1, replacement)
							}, makeGreedy$1]
						});
					} else if (lazy$1.min === 1) context.report({
						node,
						loc: getRegexpLocation(lazy$1),
						messageId: "uselessQuantifier",
						suggest: [{
							messageId: "suggestRemoveQuantifier",
							fix: fixReplaceNode$1(lazy$1, lazy$1.element.raw)
						}, makeGreedy$1]
					});
					else context.report({
						node,
						...
						data: { min: String(lazy$1.min) },
						suggest: [{
							messageId: "suggestRange",
							data: { min: String(lazy$1.min) },
							fix: fixReplaceNode$1(lazy$1, `${lazy$1.element.raw}{${lazy$1.min}}`)
						}, makeGreedy$1]
					});
				}
			} };
...
*/
function fixSimplifyQuantifier(quantifier, result, { fixReplaceNode: fixReplaceNode$1 }) {
	...
	return [replacement, fixReplaceNode$1(ancestor, () => {
...
*/
function getSingleRepeatedChar(element, flags, cache$1 = getCache(flags)) {
	let value = cache$1.get(element);
	if (value === void 0) {
		value = uncachedGetSingleRepeatedChar(element, flags, cache$1);
		cache$1.set(element, value);
	}
	return value;
}
/**
* Returns the largest character set such that `L(chars) ⊆ L(element)`.
*/
function uncachedGetSingleRepeatedChar(element, flags, cache$1) {
	switch (element.type) {
		case "Alternative": {
			let total = void 0;
			for (const e of element.elements) {
				const c = getSingleRepeatedChar(e, flags, cache$1);
		...
		case "CapturingGroup":
		case "Group": return element.alternatives.map((a) => getSingleRepeatedChar(a, flags, cache$1)).reduce((a, b) => a.union(b));
		case "Quantifier":
			if (element.max === 0) return Chars.empty(flags);
			return getSingleRepeatedChar(element.element, flags, cache$1);
		default: return assertNever(element);
	}
}
/**
			...
			*/
			function reportStartQuantifiers(capturingGroup) {
				...
				for (const quantifier of startQuantifiers) {
					const result = canSimplifyQuantifier(quantifier, flags, parser$1);
...
var no_misleading_unicode_character_default = createRule("no-misleading-unicode-character", {
	...
	create(context) {
		...
		function createVisitor(regexpContext) {
			const { node, patternSource, flags, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 } = regexpContext;
			return {
				onCharacterClassEnter(ccNode) {
					...
					context.report({
						...
						...makeFix(fixReplaceNode$1(ccNode, () => fix), "fixCharacterClass", { graphemes })
					});
				},
...
var no_missing_g_flag_default = createRule("no-missing-g-flag", {
	...
	create(context) {
		...
		function verifyExpressionReference(ref, { regexpNode, fixReplaceFlags: fixReplaceFlags$1, flagsString }) {
			...
			function buildFixer() {
				...
				return fixReplaceFlags$1(`${flagsString}g`, false);
			}
		}
		return defineRegexpVisitor(context, {
...
var no_non_standard_flag_default = createRule("no-non-standard-flag", {
	...
	create(context) {
		/** The logic of this rule */
		function visit({ regexpNode, getFlagsLocation: getFlagsLocation$1, flagsString }) {
			if (flagsString) {
...
var no_octal_default = createRule("no-octal", {
	...
	create(context) {
		function createVisitor({ node, fixReplaceNode: fixReplaceNode$1, getRegexpLocation }) {
			return { onCharacterEnter(cNode) {
...
var no_super_linear_backtracking_default = createRule("no-super-linear-backtracking", {
	...
	create(context) {
		...
		function createVisitor(regexpContext) {
			const { node, patternAst, flags, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, getUsageOfPattern: getUsageOfPattern$1 } = regexpContext;
			const result = analyse(getJSRegexppAst(regexpContext), {
				reportTypes: { Move: false },
				assumeRejectingSuffix: reportUncertain && getUsageOfPattern$1() !== UsageOfPattern.whole
			});
			for (const report of result.reports) {
				const exp = report.exponential ? " This is going to cause exponential backtracking resulting in exponential worst-case runtime behavior." : getUsageOfPattern$1() !== UsageOfPattern.whole ? " This might cause exponential backtracking." : "";
				const attack = `/${report.character.literal.source}+/${flags.ignoreCase ? "i" : ""}`;
				const fix = fixReplaceNode$1(patternAst, () => report.fix()?.source ?? null);
	...
	create(context) {
		...
		*/
		function* getSimpleReports(regexpContext, assumeRejectingSuffix) {
			...
			const parser$1 = JS.Parser.fromAst(getJSRegexppAst(regexpContext, true));
			for (const q of findReachableQuantifiers(patternAst, flags)) {
				...
				let e = parser$1.parseElement(q.element, {
					assertions: "parse",
		...
		function createVisitor(regexpContext) {
			const { node, flags, getRegexpLocation, getUsageOfPattern: getUsageOfPattern$1 } = regexpContext;
		...
		function createVisitor({ node, fixReplaceNode: fixReplaceNode$1, getRegexpLocation }) {
			return { onAssertionEnter(aNode) {
				...
				context.report({
					...
					loc: getRegexpLocation(aNode),
					messageId: "unexpected",
					fix: fixReplaceNode$1(aNode, replacement)
				});
			} };
		}
		return defineRegexpVisitor(context, { createVisitor });
...
var no_trivially_nested_quantifier_default = createRule("no-trivially-nested-quantifier", {
	...
	create(context) {
		function createVisitor({ node, fixReplaceNode: fixReplaceNode$1, fixReplaceQuant: fixReplaceQuant$1, getRegexpLocation }) {
			return { onQuantifierEnter(qNode) {
				...
				for (const child of iterateSingleQuantifiers(element)) {
					...
					if (element.alternatives.length === 1) {
						...
						context.report({
							...
							messageId: "nested",
							data: { quant: quantStr },
							fix: fixReplaceNode$1(qNode, replacement)
						});
					} else {
						...
						if (quant.min === 1 && quant.max === 1) context.report({
							...
							messageId: "childOne",
							fix: fixReplaceNode$1(child, child.element.raw)
						});
						else {
							quant.greedy = void 0;
							context.report({
								...
								fix: fixReplaceQuant$1(child, quant)
							});
						}
					}
				}
...
var no_unused_capturing_group_default = createRule("no-unused-capturing-group", {
	...
	create(context) {
		...
		function reportUnused(unused, regexpContext) {
			const { node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, getAllCapturingGroups } = regexpContext;
			if (allowNamed) {
			...
			for (const cgNode of unused) {
				const fix = fixableGroups.has(cgNode) ? fixReplaceNode$1(cgNode, cgNode.raw.replace(/^\((?:\?<[^<>]+>)?/u, "(?:")) : null;
				context.report({
					node,
					loc: getRegexpLocation(cgNode),
		...
		function getCapturingGroupReferences(regexpContext) {
			...
			return {
				...
				unusedNamedRef(name$2) {
					if (hasUnknownName) return false;
					return !namedRefs.includes(name$2);
				}
			};
		}
		function createVisitor(regexpContext) {
...
function createReorderingGetFirstCharAfter(forbidden) {
	...
	return (afterThis, direction, flags) => {
		...
		if (afterThis.parent.type === "Alternative") {
			...
			for (let i = start + inc$1; i >= 0 && i < elements.length; i += inc$1) {
...
var no_useless_assertions_default = createRule("no-useless-assertions", {
	...
	create(context) {
		function createVisitor({ node, flags, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }) {
			const reported = /* @__PURE__ */ new Set();
			function replaceWithEmptyString(assertion) {
				if (assertion.parent.type === "Quantifier") return fixReplaceNode$1(assertion.parent, "");
				return fixReplaceNode$1(assertion, "");
			}
			function replaceWithEmptySet(assertion) {
				if (assertion.parent.type === "Quantifier") {
					if (assertion.parent.min === 0) return fixReplaceNode$1(assertion.parent, "");
					const [element$1, replacement$1] = removeAlternative(assertion.parent.parent);
					return fixReplaceNode$1(element$1, replacement$1);
				}
				const [element, replacement] = removeAlternative(assertion.parent);
				return fixReplaceNode$1(element, replacement);
			}
...
function getUselessProblem(backRef, flags) {
	...
	const problems = [];
	for (const group$1 of groups) {
		const messageId$1 = getUselessMessageId(backRef, group$1, flags);
...
var no_useless_character_class_default = createRule("no-useless-character-class", {
	...
	create(context) {
		...
		function createVisitor({ node, pattern, flags, fixReplaceNode: fixReplaceNode$1, getRegexpLocation }) {
			...
			return {
				...
				onCharacterClassLeave(ccNode) {
					...
					context.report({
						...
						fix: fixReplaceNode$1(ccNode, unwrapped.join(""))
					});
					/**
...
var no_useless_escape_default = createRule("no-useless-escape", {
	...
	create(context) {
		function createVisitor({ node, flags, pattern, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }) {
			function report(cNode, offset, character, fix) {
				context.report({
					...
					data: { character },
					fix: fix ? fixReplaceNode$1(cNode, character) : null
				});
			}
			const characterClassStack = [];
			return {
...
*/
function fixRemoveFlag({ flagsString, fixReplaceFlags: fixReplaceFlags$1 }, flag) {
	if (flagsString) return fixReplaceFlags$1(flagsString.replace(flag, ""));
	return null;
}
/**
...
function createUselessGlobalFlagVisitor(context, strictTypes) {
	let ReportKind = /* @__PURE__ */ function(ReportKind$1) {
		ReportKind$1[ReportKind$1["usedOnlyInSplit"] = 0] = "usedOnlyInSplit";
		ReportKind$1[ReportKind$1["usedOnlyInSearch"] = 1] = "usedOnlyInSearch";
		ReportKind$1[ReportKind$1["usedOnlyOnceInExec"] = 2] = "usedOnlyOnceInExec";
		ReportKind$1[ReportKind$1["usedOnlyOnceInTest"] = 3] = "usedOnlyOnceInTest";
		ReportKind$1[ReportKind$1["unused"] = 4] = "unused";
		return ReportKind$1;
	}({});
	/**
	...
	*/
	function reportUselessGlobalFlag(regExpReference, data) {
		const { getFlagLocation: getFlagLocation$1 } = regExpReference.regExpContext;
		const node = regExpReference.defineNode;
		context.report({
			node,
			loc: getFlagLocation$1("g"),
...
var no_useless_non_capturing_group_default = createRule("no-useless-non-capturing-group", {
	...
	create(context) {
		...
		function createVisitor({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, getUsageOfPattern: getUsageOfPattern$1 }) {
			let isIgnored;
			if (allowTop === "always") isIgnored = isTopLevel;
			else if (allowTop === "partial") if (getUsageOfPattern$1() !== UsageOfPattern.whole) isIgnored = isTopLevel;
			...
			return { onGroupEnter(gNode) {
				...
				context.report({
					...
					messageId: "unexpected",
					fix: fixReplaceNode$1(gNode, () => {
						if (allowTop === "never" && isTopLevel(gNode) && getUsageOfPattern$1() !== UsageOfPattern.whole) return null;
...
var no_useless_quantifier_default = createRule("no-useless-quantifier", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, flags, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 } = regexpContext;
			/**
			* Returns a fix that replaces the given quantifier with its
			* quantified element
			*/
			function fixRemoveQuant(qNode) {
				return fixReplaceNode$1(qNode, () => {
		...
		function createVisitor(regexpContext) {
			...
			function fixRemoveExpression(expr) {
				if (expr.parent.type === "ExpressionCharacterClass") {
					...
					return fixReplaceNode$1(cc, cc.negate ? "[^]" : "[]");
				}
				return fixReplaceNode$1(expr, "[]");
			}
			return {
				onClassIntersectionEnter(iNode) {
					...
					if (leftSet.isSubsetOf(rightSet)) {
						context.report({
							...
							fix: fixReplaceNode$1(iNode, iNode.left.raw)
						});
						return;
					}
					if (rightSet.isSubsetOf(leftSet)) {
						context.report({
							...
							fix: fixReplaceNode$1(iNode, iNode.right.raw)
						});
						return;
					}
					...
					for (const e of [...toRemoveRight, ...toRemoveLeft]) context.report({
						...
						data: { expr: e.raw },
						fix: fixReplaceNode$1(iNode, removeDescendant(iNode, e))
					});
				},
				onClassSubtractionEnter(sNode) {
					...
					if (leftSet.isDisjointWith(rightSet)) {
						context.report({
							...
							fix: fixReplaceNode$1(sNode, sNode.left.raw)
						});
						return;
					}
					if (leftSet.isSubsetOf(rightSet)) {
					...
					for (const e of toRemove) context.report({
						...
						messageId: "subtractionRemove",
						data: { expr: e.raw },
						fix: fixReplaceNode$1(sNode, removeDescendant(sNode, e))
					});
				}
			};
		}
...
var no_useless_string_literal_default = createRule("no-useless-string-literal", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, pattern } = regexpContext;
			return { onStringAlternativeEnter(saNode) {
					...
					context.report({
						...
						fix: fixReplaceNode$1(csdNode, () => {
							...
							if (csdNode.parent.type === "ClassIntersection" || csdNode.parent.type === "ClassSubtraction") {
								const escape$2 = saNode.raw === "^" ? "\\" : "";
								return String.raw`[${escape$2}${saNode.raw}\q{${alternativesText}}]`;
							}
							const escape$1 = isNeedEscapeForAdjacentPreviousCharacter(csdNode, saNode) ? "\\" : "";
							return String.raw`${escape$1}${saNode.raw}\q{${alternativesText}}`;
						})
					});
				}
			} };
...
var no_useless_two_nums_quantifier_default = createRule("no-useless-two-nums-quantifier", {
	...
	create(context) {
		function createVisitor({ node, getRegexpLocation, fixReplaceQuant: fixReplaceQuant$1 }) {
			return { onQuantifierEnter(qNode) {
				if (qNode.min === qNode.max) {
					...
					context.report({
						...
						messageId: "unexpected",
						data: { expr: text },
						fix: fixReplaceQuant$1(qNode, `{${qNode.min}}`)
					});
				}
			} };
		}
...
var no_zero_quantifier_default = createRule("no-zero-quantifier", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, patternAst } = regexpContext;
			return { onQuantifierEnter(qNode) {
...
var CapturingGroupReporting = /* @__PURE__ */ function(CapturingGroupReporting$1) {
	CapturingGroupReporting$1["ignore"] = "ignore";
	CapturingGroupReporting$1["report"] = "report";
	return CapturingGroupReporting$1;
				...
				onAlternativeLeave(aNode) {
					for (let i = 0; i < aNode.elements.length - 1; i++) {
						...
						if (replacement.type === "Both") context.report({
							...
							fix: fixReplaceNode$1(replacement.nested, () => {
								if (involvesCapturingGroup) return null;
		...
		function createVisitor(regexpContext) {
			...
			*/
			function fixReplaceAlternatives(n, newAlternatives) {
				...
				return fixReplaceNode$1(n, prefix + newAlternatives + suffix);
			}
			/**
			...
			function verifyCharacterClass(ccNode) {
				...
				if (predefined) {
					context.report({
						...
						fix: fixReplaceNode$1(ccNode, predefined)
					});
					return;
				}
				...
				for (const e of ccNode.elements) if (isDigits(e) && e.raw !== expected) context.report({
					...
					fix: fixReplaceNode$1(e, expected)
				});
			}
			return {
...
var SideEffect = /* @__PURE__ */ function(SideEffect$1) {
	SideEffect$1[SideEffect$1["startRef"] = 0] = "startRef";
	SideEffect$1[SideEffect$1["endRef"] = 1] = "endRef";
	return SideEffect$1;
}(SideEffect || {});
/**
	...
	create(context) {
		...
		function createVisitor(regexpContext) {
			...
			for (const ref of extractExpressionReferences(regexpNode, context)) if (ref.type === "argument") {
				...
				const replaceReference$1 = getReplaceReferenceFromCallExpression(ref.callExpression);
				if (!replaceReference$1) return {};
				replaceReferenceList.push(replaceReference$1);
		...
		function createVisitor({ node, fixReplaceNode: fixReplaceNode$1, getRegexpLocation }) {
			return { onBackreferenceEnter(bNode) {
				if (!bNode.ambiguous && bNode.resolved.name && !bNode.raw.startsWith("\\k<")) context.report({
					...
					messageId: "unexpected",
					fix: fixReplaceNode$1(bNode, `\\k<${bNode.resolved.name}>`)
				});
			} };
		}
		return defineRegexpVisitor(context, { createVisitor });
...
var prefer_question_quantifier_default = createRule("prefer-question-quantifier", {
	...
	create(context) {
		function createVisitor({ node, getRegexpLocation, fixReplaceQuant: fixReplaceQuant$1, fixReplaceNode: fixReplaceNode$1 }) {
			return {
				onQuantifierEnter(qNode) {
				...
				onGroupEnter(gNode) {
					if (!gNode.alternatives[gNode.alternatives.length - 1].elements.length) {
						...
						context.report({
							...
							fix: fixReplaceNode$1(reportNode, instead)
						});
					}
				}
			};
...
var prefer_result_array_groups_default = createRule("prefer-result-array-groups", {
	...
	create(context) {
		...
		function isRegExpMatchArrayOrRegExpExecArray(tsType) {
			if (isClassOrInterface(tsType)) {
				const name$2 = tsType.symbol.escapedName;
				return name$2 === "RegExpMatchArray" || name$2 === "RegExpExecArray";
			}
			return false;
		}
	}
...
var prefer_w_default = createRule("prefer-w", {
	...
	create(context) {
		function createVisitor({ node, flags, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, patternSource }) {
			return { onCharacterClassEnter(ccNode) {
				const charSet = toUnicodeSet(ccNode, flags);
				let predefined = void 0;
				const word = Chars.word(flags);
...
var require_unicode_regexp_default = createRule("require-unicode-regexp", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, flags, flagsString, getFlagsLocation: getFlagsLocation$1, fixReplaceFlags: fixReplaceFlags$1 } = regexpContext;
			if (flagsString === null) return {};
			if (!flags.unicode && !flags.unicodeSets) context.report({
				node,
				loc: getFlagsLocation$1(),
				messageId: "require",
				fix: fixReplaceFlags$1(() => {
			...
			*/
			function reportWhenFixedIsCompatible({ reportNode, targetNode, messageId, data, fix }) {
				...
				context.report({
					...
					messageId,
					data: data || {},
					fix: fixReplaceNode$1(targetNode, fixedText)
				});
				return true;
			}
...
*/
function cachedApproximateLexicographicallySmallest(alternative, parser$1, flags) {
	let cached = lssCache.get(alternative);
	if (cached === void 0) {
		cached = approximateLexicographicallySmallest(alternative, parser$1, flags);
...
*/
function approximateLexicographicallySmallest(alternative, parser$1, flags) {
	const lss = getLexicographicallySmallestFromAlternative(alternative, parser$1, flags);
...
*/
function getLexicographicallySmallestFromAlternative(alternative, parser$1, flags) {
	...
	try {
		const result = parser$1.parseElement(alternative, {
			assertions: "unknown",
			backreferences: "disable",
...
*/
function sortAlternatives(alternatives, parser$1, flags) {
	alternatives.sort((a, b) => {
		const lssDiff = compareWords$1(cachedApproximateLexicographicallySmallest(a, parser$1, flags), cachedApproximateLexicographicallySmallest(b, parser$1, flags));
...
* see https://github.com/ota-meshi/eslint-plugin-regexp/pull/423.
*/
function sortStringAlternatives(alternatives, parser$1, flags) {
	alternatives.sort((a, b) => {
		return compareWords$1(getLexicographicallySmallestFromAlternative(a, parser$1, flags), getLexicographicallySmallestFromAlternative(b, parser$1, flags));
	});
}
/**
...
var sort_alternatives_default = createRule("sort-alternatives", {
	...
	create(context) {
		...
		function createVisitor(regexpContext) {
			const { node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1, flags } = regexpContext;
			...
			function trySortRun(run) {
				...
				if (canReorder(alternatives, flags)) {
					sortAlternatives(alternatives, parser$1, flags);
			...
			*/
			...
			function onClassStringDisjunction(parent) {
				...
				sortStringAlternatives(alternatives, parser$1, flags);
				trySortNumberAlternatives(alternatives);
				enforceSorted({
					startIndex: 0,
...
var sort_flags_default = createRule("sort-flags", {
	...
	create(context) {
		...
		function visit({ regexpNode, flagsString, ownsFlags, getFlagsLocation: getFlagsLocation$1, fixReplaceFlags: fixReplaceFlags$1 }) {
			if (flagsString && ownsFlags) {
				...
				if (flagsString !== sortedFlags) context.report({
					...
					fix: fixReplaceFlags$1(sortedFlags, false)
				});
			}
		}
		return defineRegexpVisitor(context, {
...
var strict_default = createRule("strict", {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, flags, pattern, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 } = regexpContext;
			if (flags.unicode || flags.unicodeSets) return {};
			let reported = false;
			...
			function report(messageId, element, fix) {
				...
				if (fix && typeof fix === "object") context.report({
					...
					suggest: [{
						...
						fix: fixReplaceNode$1(element, fix.fix)
					}]
				});
				else context.report({
					node,
					...
					messageId,
					data: { expr: mention(element) },
					fix: fix ? fixReplaceNode$1(element, fix) : null
				});
			}
			return {
				onCharacterEnter(cNode) {
...
var unicode_escape_default = createRule("unicode-escape", {
	...
	create(context) {
		...
		function verifyForUnicodeCodePointEscape({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }, kind, cNode) {
			...
			context.report({
				...
				fix: fixReplaceNode$1(cNode, unicodeCodePointEscape)
			});
		}
		function verifyForUnicodeEscape({ node, getRegexpLocation, fixReplaceNode: fixReplaceNode$1 }, kind, cNode) {
			...
			context.report({
				...
				fix: fixReplaceNode$1(cNode, unicodeEscape)
			});
		}
		...
		function createVisitor(regexpContext) {
			...
			function onUnicodeProperty(cs) {
				...
				function fixReplace(inner) {
					return fixReplaceNode$1(cs, `${cs.raw.slice(0, 2)}{${inner}}`);
				}
...
var use_ignore_case_default = createRule("use-ignore-case", {
	meta: {
		docs: {
	...
	create(context) {
		function createVisitor(regexpContext) {
			const { node, flags, ownsFlags, flagsString, patternAst, patternSource, getUsageOfPattern: getUsageOfPattern$1, getFlagsLocation: getFlagsLocation$1, fixReplaceFlags: fixReplaceFlags$1 } = regexpContext;
			if (!ownsFlags || flagsString === null) return {};
			if (flags.ignoreCase) return {};
			if (getUsageOfPattern$1() === UsageOfPattern.partial) return {};
				...
				onPatternLeave() {
					...
					context.report({
						...
						fix(fixer) {
							...
							const flagsFix = fixReplaceFlags$1(getIgnoreCaseFlagsString(flagsString), false)(fixer);
...
//#endregion
//#region package.json
var name$1 = "eslint-plugin-regexp";
var version$1 = "3.0.0";
...
//#endregion
export { configs, lib_default as default, meta, rules };