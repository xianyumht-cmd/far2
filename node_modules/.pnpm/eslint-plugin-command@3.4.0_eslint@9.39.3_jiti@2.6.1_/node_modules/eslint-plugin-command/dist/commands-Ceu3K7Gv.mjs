...
const keepAligned = {
	...
	action(ctx) {
		...
		for (let i = 0; i < n && i < 20; i++) {
			...
			const maxIndex = lines.reduce((maxIndex$1, line) => Math.max(line.indexOf(symbol, lastPos), maxIndex$1), -1);
...
function sort(ctx, node, list, getName, insertComma = true) {
	...
	for (const [start, end] of segments) reordered.splice(start, end - start, ...reordered.slice(start, end).sort((a, b) => {
		...
		for (let i = 0; i < length; i++) {
			const a$1 = nameA[i];
			const b$1 = nameB[i];
			if (a$1 == null || b$1 == null || a$1 === b$1) continue;
...
const noXAbove = {
	name: "no-x-above",
	match: /* @__PURE__ */ new RegExp(`^\\s*[/:@]\\s*no-(${types.join("|")})-(above|below)$`),
	action(ctx) {
