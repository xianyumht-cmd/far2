...
async function refresh() {
  ...
  errorText.value = ''
  try {
    const response = await api.get('/api/activities', { timeout: 15000 })
    if (!response.data?.ok)
  ...
  <div class="space-y-4">
    <div class="flex flex-col gap-3 rounded-lg bg-white p-4 shadow sm:flex-row sm:items-center sm:justify-between dark:bg-gray-800">
      <div>
        ...
        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          自动读取 ActivityService.List。当前框架不包含 Operate，不购买、不刷新、不抽奖、不领取活动奖励。
        </div>
      </div>
    ...
    <template v-if="overview">
      ...
      <div class="rounded-lg border border-blue-200 bg-blue-50/70 px-4 py-3 text-xs text-blue-700 dark:border-blue-900/40 dark:bg-blue-950/20 dark:text-blue-300">
        Transport: {{ overview.framework.transport }} · adapters: {{ overview.framework.adapters.length }} · Operate: {{ overview.framework.operateEnabled ? 'enabled' : 'disabled' }}
      </div>
