...
class ResidentCdpBridge {
    constructor(options = {}) {
        ...
        // FAR2-WX-CONTEXT-RECOVERY-v1
        // WMPF may reconnect without replaying Runtime.executionContextCreated.
        // Historical IDs are hints only and are always re-validated on the
        // currently connected runtime before becoming selected.
        this.contextHints = new Map();
    ...
    onMessage(data) {
        ...
        if (message && message.method === 'Runtime.executionContextCreated' && message.params && message.params.context) {
            ...
            if (id > 0) {
                ...
                this.contextHints.set(id, entry);
            }
        }
...
async function selectFarmContext(bridge) {
    ...
    // FAR2-WX-CONTEXT-RECOVERY-v1
    // WMPF can reconnect without replaying Runtime.executionContextCreated.
    // Give it a short event window, then validate current IDs, historical hints,
    // and finally a small bounded set of likely context IDs.
    const contextDeadline = Date.now() + 900;
    while (bridge.isConnected() && bridge.contexts.size === 0 && Date.now() < contextDeadline) {
        await sleep(60);
    }
    if (!bridge.isConnected()) {
        throw createError('wechat_resident_runtime_disconnected', 'Resident farm runtime disconnected while selecting context');
    }
    ...
    for (const ctx of [...bridge.contexts.values()].sort((a, b) => a.id - b.id).slice(0, 64)) {
        addCandidate(ctx.id);
    }

    if (bridge.contextHints && typeof bridge.contextHints.values === 'function') {
        for (const ctx of [...bridge.contextHints.values()].sort((a, b) => a.id - b.id).slice(0, 64)) {
            addCandidate(ctx.id);
        }
    }

    for (let id = 1; id <= 32; id += 1) addCandidate(id);

    for (const id of candidates) {
        if (!bridge.isConnected()) {
            throw createError('wechat_resident_runtime_disconnected', 'Resident farm runtime disconnected while probing context');
        }

        let value = null;
        try { value = await bridge.evaluate(infoExpr, id, false, 1200); } catch {}
        if (value && value.hasLogin && value.appId === EXPECTED_APP_ID) {
            const entry = bridge.contexts.get(id) || { id, name: '', origin: '' };
            if (bridge.contextHints && typeof bridge.contextHints.set === 'function') {
                bridge.contextHints.set(id, entry);
            }
            return {
                id,
    ...
    throw createError(
        bridge.contexts.size > 0 ? 'wechat_resident_target_context_missing' : 'wechat_resident_context_probe_miss',
        'Exact farm AppId context with wx.login was not found on the connected runtime'
    );
}
