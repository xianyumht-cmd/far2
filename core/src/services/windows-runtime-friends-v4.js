...
const MARKER = '/*__FAR2_FRIEND_CAPTURE_V4__*/';
const ARTIFACT_NAME = '_far2_friend_frames_v4.jsonl';
...
function patchGameFiles(options = {}) {
    ...
    for (const folder of folders) {
        ...
        if (!gameJs) continue;
        const backup = `${gameJs}.far2-friend-capture-v4.bak`;
...
async function decodeGateEntry(entry) {
    ...
    if (entry.direction === 'o' && /SyncAll/i.test(methodName)) {
        let decoded = null;
        try { decoded = p.SyncAllRequest.decode(raw); } catch {}
        if (!decoded) {
            try {
                const plain = await cryptoWasm.decryptBuffer(raw);
                decoded = p.SyncAllRequest.decode(plain);
            } catch {}
        }
...
async function waitForData(timeoutMs) {
    ...
    while (Date.now() < deadline) {
        const before = openIds.size + gids.size;
        ...
        if (openIds.size + gids.size > before) lastNewAt = Date.now();
        if ((openIds.size || gids.size) && lastNewAt && Date.now() - lastNewAt >= 2200) break;
        await sleep(200);
    }
    return {
        gids: [...gids],
...
async function doCapture(options = {}) {
    ...
    try {
        ...
        if (log) log(`已临时注入 ${patched.length} 个 QQ 农场缓存用于好友采集 V4`);
        ...
        return { ...captured, source: 'windows_qq_runtime_friend_capture_v4' };
    } finally {
