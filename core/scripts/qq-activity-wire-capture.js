...
function buildPayload() {
    return `${MARKER}\n;(function(){\n` +
`if(globalThis.__far2ActivityWireInstalled)return;globalThis.__far2ActivityWireInstalled=true;\n` +
`var FILE='/${CAPTURE_FILE}',SERVICE='${SERVICE}',READ={List:1,GetGroup:1};\n` +
...
function restoreGameFiles(patched) {
    for (const item of patched || []) {
        try {
            ...
            if (fs.existsSync(item.backup)) fs.unlinkSync(hitem.backup);
        } catch (error) {
            console.warn(`恢复失败: ${item.gameJs} -> ${error.message}`);
        }
    }
}
...
async function decodeRows(rows) {
    ...
    for (const [method, bodies] of byMethod.entries()) {
        ...
        for (const cipherHex of bodies) {
            if (!cipherHex) {
                decodedBodies.push({ cipherLength: 0, cipherHex: '', plainLength: 0, plainHex: '', wire: [] });
                continue;
            }
            ...
            try {
                ...
                decodedBodies.push({ cipherLength: cipher.length, cipherHex, decryptError: error.message });
            }
        }
...
async function main() {
    if (process.platform !== 'win32') throw new Error('该捕获器仅支持 Windows QQ。');
    ...
    try {
        ...
        console.log('只捕获 ActivityService 且自动排除 List/GetGroup；不记录登录/鉴权流量。');
        console.log(`已临时注入 ${patched.length} 个最近 QQ 农场缓存，退出前会恢复。`);
        openFarmMiniApp();
        await waitEnter('\n请在官方农场里只完成你想取证的一个活动动作（领取/兑换/免费抽等），完成后回到这里按 Enter...');
    ...
    console.log(`\n捕获请求: ${rows.length}`);
    for (const row of decoded) console.log(`- ${row.method}: ${row.uniqueBodyCount} 个唯一 body`);
