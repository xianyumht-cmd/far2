...
function main() {
    ...
    console.log('✅ same-object seed_id + size=0 resolves to 1x1 PASS');
    ...
    console.log('✅ same-object seed_id + size=2 resolves to 2x2 PASS');

    const ambiguous = '{"seed_id":20903,"size":0,"nested":{"size":2}}';
    assert.equal(parsePlantObjectText(ambiguous, 20903), null);
    console.log('✅ conflicting size values fail closed PASS');

    const unrelated = '{"item_id":20904,"size":0}';
    assert.equal(parsePlantObjectText(unrelated, 20904), null);
