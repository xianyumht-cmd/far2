...
async function loginPcAndBuildJar() {
    console.log('\n=== STEP 1: PC QQ QR LOGIN ===');
    const created = await QRLoginSession.requestQRCode('qzone');
    saveAndOpenQr(created.qrcode, 'qq-farm-pc-bridge');
    ...
    while (Date.now() < deadline) {
        ...
        console.log(`check_sig: ${status.jumpUrl ? '已获取' : '未获取'}`);
        if (!status.jumpUrl) throw new Error('未获得 check_sig 跳转地址');

        const jar = createCookieJar();
        jar.set('qrsig', created.qrsig);
        mergeSetCookie(jar, status.cookie);

        console.log(`跟随 check_sig 前 Cookie 名称: ${cookieNames(jar).join(', ') || '(none)'}`);
        ...
        console.log(`最终地址: ${followed.finalUrl}`);
        console.log(`QQ 登录态 Cookie 名称: ${cookieNames(jar).join(', ') || '(none)'}`);
        console.log('说明: 仅打印 Cookie 名称，不打印 Cookie 值；Cookie 只存在当前进程内存。');
...
async function tryMiniappBridge(jar) {
    console.log('\n=== STEP 2: QQ SESSION -> MINIAPP AUTH BRIDGE ===');
    const created = await MiniProgramLoginSession.requestLoginCode();
    console.log(`已生成 miniapp 登录会话: ${mask(created.code)}`);
    console.log('尝试用刚才的 QQ 网页登录态直接访问 miniapp 确认页...');

    try {
        const followed = await followWithJar(created.url, jar, {
            referer: 'https://q.qq.com/',
            maxHops: 8,
        });
        console.log(`miniapp 确认页访问完成: HTTP ${followed.status}`);
        console.log(`确认页最终地址: ${followed.finalUrl}`);
        console.log(`当前 Cookie 名称: ${cookieNames(jar).join(', ') || '(none)'}`);
    } catch (err) {
        console.log(`miniapp 确认页访问异常: ${err.message}`);
    }

    console.log('轮询 miniapp ticket...');
    ...
    while (Date.now() < deadline) {
        ...
        if (state === 'Used') {
            console.log('\nminiapp 会话已失效。');
            return { ok: false, reason: 'miniapp_used' };
        }
        if (state !== 'OK') {
            console.log(`\nminiapp 状态异常: ${status && status.msg ? status.msg : state}`);
            return { ok: false, reason: `miniapp_${state}` };
        }

        console.log(`\nminiapp ticket 已获得，QQ=${status.uin || '(unknown)'}`);
        return { ok: true, ticket: status.ticket, uin: status.uin || '' };
    }

    console.log('\nPC 登录态没有自动确认 miniapp 登录会话。');
    return { ok: false, reason: 'miniapp_bridge_wait_timeout' };
}

async function exchangeFarmCode(ticket, jar) {
    console.log('\n=== STEP 3: TICKET -> FARM CODE ===');
...
async function main() {
    console.log('QQ Farm PC-Bridge Tester');
    console.log(`Farm WS 探测版本: ${probeVersion}`);
    console.log('用途: 验证 PC QQ 扫码登录态能否桥接到 miniapp，再换取有效 Farm Code。');
    console.log('安全: 不写入账号、不启动挂机、不落盘保存 QQ Cookie。');

    const pc = await loginPcAndBuildJar();
    const bridge = await tryMiniappBridge(pc.jar);

    let result;
    if (!bridge.ok) {
        result = { ok: false, reason: bridge.reason, uin: '' };
    } else {
        const exchanged = await exchangeFarmCode(bridge.ticket, pc.jar);
        result = { ...exchanged, uin: bridge.uin || '' };
    }
    ...
    if (result.ok) {
        console.log('\n✅ PC 扫码登录态成功桥接为可通过 Farm WebSocket 握手的 Code。');
    } else {
        console.log('\n❌ PC-Bridge 当前没有得到可用 Farm Code。');
        console.log('请把 STEP 1~3 和 RESULT 的输出发回来；不要发送任何 Cookie 值。');
    }
}
