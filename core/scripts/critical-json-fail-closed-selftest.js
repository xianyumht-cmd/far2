...
function expectCritical(filePath, expectedReason, fallback = () => ({})) {
    let error = null;
    try { readJsonFile(filePath, fallback); } catch (err) { error = err; }
    assert.ok(error, `expected critical failure: ${expectedReason}`);
...
function main() {
    ...
    try {
        ...
        const malformed = expectCritical(accounts, 'invalid_json', () => ({ accounts: [], nextId: 1 }));
        assert.ok(malformed.backupPath && fs.existsSync(malformed.backupPath), 'corrupt bytes must be preserved');
        assert.equal(fs.readFileSync(malformed.backupPath, 'utf8'), '{"accounts":[{"id":"1"}],');
        ...
        expectCritical(accounts, 'empty_file');

        fs.writeFileSync(accounts, '{}', 'utf8');
        const shape = expectCritical(accounts, 'invalid_shape');
        assert.ok(shape.backupPath && fs.existsSync(shape.backupPath), 'invalid-shape bytes must be preserved');
        assert.equal(fs.readFileSync(shape.backupPath, 'utf8'), '{}');
        ...
        console.log('✅ malformed/empty/invalid-shape accounts.json fails closed and preserves backup');
