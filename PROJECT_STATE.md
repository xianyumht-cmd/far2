...
## 2026-08-14 priority override

P7E 狗粮/宠物养成当前链已完成并通过真实 E2E（PR #46）。后续不再继续扩宠物功能，除非出现回归或用户明确提出新需求。

当前优先级固定为：

1. **好友巡查可观测性 / 经验满后的帮助空转修复**：偷菜本身已确认正常，不重写偷菜核心；经验满后只有当前确实需要帮助的护主犬好友才继续高频帮助，空目标进入低频 backoff；偷菜补低噪音状态日志。
2. **背包已有活动种子识别与优先种植**：覆盖 1x1 / 2x2，优先自动发现/学习，未知映射 fail-closed，不再因为未识别活动种子而直接购买普通商店种子。
3. **活动自动发现、参加与领取**：优先从只读活动结构自动发现/分类，只有已证明写协议的活动类型才自动执行，未知活动继续只读取证。

详细交接：`docs/PRIORITY_HANDOFF_2026-08-14.md`。
