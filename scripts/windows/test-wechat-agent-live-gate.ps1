...
$gateScript = Join-Path $coreRoot 'scripts\wechat-agent-live-gate.js'
$tempRoot = Join-Path $env:TEMP 'FAR2-WeChat-CDP'
$depsRoot = Join-Path $tempRoot 'p4-node-deps'
...
function Get-NodePath {
    ...
    throw 'Node.js 22+ was not found. Run the earlier WeChat P3/P4 gate once.'
}
...
function Test-Dependencies {
    ...
    try {
        & $NodePath -e "require('ws');require('long');require('protobufjs');require('node-fetch');" *> $null
...
function Install-Dependencies {
    ...
    if (Test-Path -LiteralPath $npmCli) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "P5 live gate dependency install failed with exit code $LASTEXITCODE." }
        return
    }
...
if (-not (Test-Path -LiteralPath $gateScript)) { throw "P5 live gate script not found: $gateScript" }
...
if (-not (Test-Dependencies -NodePath $nodePath)) {
    Write-Host 'Preparing isolated P5 live-gate Node dependencies...' -ForegroundColor Yellow
    Install-Dependencies -NodePath $nodePath
    Add-NodePath -NodeModulesPath $nodeModules
    if (-not (Test-Dependencies -NodePath $nodePath)) {
        throw 'P5 live gate dependencies are unavailable after isolated install.'
    }
}

Write-Host ''
Write-Host 'FAR2 WeChat P5 live Agent / Provider runner' -ForegroundColor Cyan
Write-Host ("Node: {0}" -f $nodePath)
Write-Host 'Transport: temporary pinned WMPF diagnostic bridge (live gate only)' -ForegroundColor DarkGray
