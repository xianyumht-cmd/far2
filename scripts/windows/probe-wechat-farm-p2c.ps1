...
function Get-WeChatProcesses {
    ...
    Get-CimInstance Win32_Process | Where-Object {
        ...
        $safeAppId = $null
        if ($null -ne $rawAppId -and $rawAppId -match '^wx[0-9a-fA-F]{16}$') {
            $safeAppId = $rawAppId.ToLowerInvariant()
        }
...
if (-not ('Far2WechatP2cWindowProbe' -as [type])) {
    Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
using System.Text;
public static class Far2WechatP2cWindowProbe {
...
function Get-FarmHostPid {
    ...
    $callback = [Far2WechatP2cWindowProbe+EnumWindowsProc]{
        param([IntPtr]$hWnd, [IntPtr]$lParam)
        [uint32]$windowProcessId = 0
        [void][Far2WechatP2cWindowProbe]::GetWindowThreadProcessId($hWnd, [ref]$windowProcessId)
        if (-not $pidSet.Contains($windowProcessId)) { return $true }
        if (-not [Far2WechatP2cWindowProbe]::IsWindowVisible($hWnd)) { return $true }
        $buf = New-Object System.Text.StringBuilder 1024
        [void][Far2WechatP2cWindowProbe]::GetWindowText($hWnd, $buf, $buf.Capacity)
        if ($buf.ToString() -eq $FarmWindowTitle) { [void]$found.Add([int]$windowProcessId) }
        return $true
    }
    [void][Far2WechatP2cWindowProbe]::EnumWindows($callback, [IntPtr]::Zero)
...
try {
    Write-Host ''
    Write-Host 'FAR2 WeChat Farm P2C WMPF AppId Probe' -ForegroundColor Cyan
    Write-Host '====================================='
    Write-Host 'Keep desktop WeChat logged in and keep QQ Classic Farm OPEN.'
    Write-Host 'This probe outputs only valid WMPF AppId strings (wx + 16 hex chars).'
    ...
    $report = [ordered]@{
        version = 1
        phase = 'wechat-farm-p2c-wmpf-appid'
        ...
        safety = [ordered]@{
            ...
            onlyValidatedWmpfAppIdsStored = $true
        }
        summary = [ordered]@{
            farmHostPid = $hostPid
    ...
    if ([string]::IsNullOrWhiteSpace($OutputPath)) {
        ...
        $OutputPath = Join-Path $dir ("wechat-farm-p2c-{0}.json" -f $stamp)
    }
    ...
    Write-Host ("Processes with validated wmpf-appid: {0}" -f @($appIdProcesses).Count)
